...
(function($) {
	...
	jQuery.fn.toc = function(options) {
		...
		my.getSlug = function(text, i) {
			var id = my.convertToSlug(text);
			i = i || 1;
			if(my.isInToc(id)) {
				i++;
				return my.getSlug(text+' '+i, i);
			}
			return id;
		};

		my.generateToc = function() {
			my.$parent.find(':header').each(function() {
				var id = my.getSlug($(this).text());
				$(this).attr('id', id);
				my.toc.push({ level: $(this).prop('tagName'), content: $(this).text(), id: id });
			});
		};

		my.tocToHtml = function() {
			var html = [];
			if(my.toc.length > 0) {
				$.each(my.toc, function(i, toc) {
					html.push('<li><a class="list-group-item level-'+toc.level.toLowerCase()+'" href="#'+toc.id+'">'+toc.content+'</a></li>');
				});
				my.$target.html(html.join(''));
			}
			else {
				my.$target.parent().hide();
			}
		};
...
(function($) {
	...
	$.fn.avoidWidows = function() {
		return this.each(function() {
			var h2all = $(this).text();
			if(h2all.search(/ /) > 0) {
				var h2a = h2all.slice(0, h2all.lastIndexOf(' '));
				var h2b = '&nbsp;' + h2all.slice(h2all.lastIndexOf(' ')+1);
				$(this).html(h2a + h2b);
			}
		});
	};
